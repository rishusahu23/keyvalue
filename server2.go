...
func main() {
	...
	config.LocalID = raft.ServerID("server2") // Set a unique LocalID for the node

	store, err := raftboltdb.NewBoltStore("raft2.db")
	if err != nil {
		log.Fatal(err)
	}

	logStore, err := raftboltdb.NewBoltStore("raft-log2.db")
	if err != nil {
		log.Fatal(err)
	}
	...
	address := ":49153"
	//advertiseAddress := "127.0.0.1:8081" // Replace with a proper address if needed

	transport, err := raft.NewTCPTransport(address, &net.TCPAddr{IP: net.ParseIP("127.0.0.1"), Port: 49153}, 3, 5*time.Second, os.Stderr)
	if err != nil {
		log.Fatal(err)
	}
	...
	bootstrapConfig := raft.Configuration{
		Servers: []raft.Server{
			{ID: "server2", Address: transport.LocalAddr()},
		},
	}
	...
	http.HandleFunc("/get", func(w http.ResponseWriter, req *http.Request) {
		...
		if node, _ := ring.GetNode(key); node == "server2" {
